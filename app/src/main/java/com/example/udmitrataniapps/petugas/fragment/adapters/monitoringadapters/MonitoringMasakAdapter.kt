...
class MonitoringMasakAdapter(val callback: Callback) : RecyclerView.Adapter<MonitoringMasakAdapter.MyViewHolder>() {
    ...
    inner class MyViewHolder(itemView: View, callback: Callback) : RecyclerView.ViewHolder(itemView) {
        fun bind(monitoring: Pesanan) {
            ...
            if (monitoring.status_pesanan == "Fase Berbunga")
                itemView.tv_fase.text = "Fase Masak"
