...
class MonitoringVegetatifAdapter(val callback: Callback) : RecyclerView.Adapter<MonitoringVegetatifAdapter.MyViewHolder>(){
    ...
    inner class MyViewHolder(itemView: View, callback: Callback) : RecyclerView.ViewHolder(itemView) {
        fun bind(monitoring: Pesanan) {
            ...
            if (monitoring.status_pesanan == "Fase Pendahuluan")
                itemView.tv_fase.text = "Fase Vegetatif"
